// chart.js
// Shared scatter-plot rendering module.
// Each graph script only needs to call scatter({...}) with its config.
...
const scores = d3.csv("./data/score.csv").then((rows) =>
    rows.map((d, i) => ({
        ...
        index: i + 1, // CSV line number minus the header line
    }))
);
...
export function scatter(config) {
    ...
    scores.then(function (rows) {

        /* Derive this chart's records without mutating the shared rows */
        const data = rows.map((d) => ({...d, value: config.value(d)}));
